sap.ui.define([
	...
	return BaseController.extend("myCompany.myApp.controller.Worklist", {
		...
		onInit: function() {
			...
			// this.setModel("issueType",this.getOwnerComponent().getModel().read("/IssueTypeSet",{
			// 	success: function(oData){
			// 		//alert("worklist init" + oData);
			// 	}
			// }));
			// this.getModel().read("/IssueTypeSet",{
			// 	success:function(oData){
			// 		alert(oData);
			// 	}

			// });
			//this.setModel("issueType",oIssueTypeModel);
			...
			this._mFilters = {
				"S01": [new sap.ui.model.Filter("Status", "EQ", "01")],
				"S02": [new sap.ui.model.Filter("Status", "EQ", "02")],
				"S03": [new sap.ui.model.Filter("Status", "EQ", "03")],
				"all": []
			};
		...
		onUpdateFinished: function(oEvent) {
			...
			if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
				...
				this.getModel().read("/issueSet/$count", {
					success: function(oData) {
				...
				this.getModel().read("/issueSet/$count", {
					success: function(oData) {
						oViewModel.setProperty("/S01", oData);
					},
					filters: this._mFilters.S01
				});

				// read the count for the outOfStock filter
				this.getModel().read("/issueSet/$count", {
					success: function(oData) {
						oViewModel.setProperty("/S02", oData);
					},
					filters: this._mFilters.S02
				});

				// read the count for the shortage filter
				this.getModel().read("/issueSet/$count", {
					success: function(oData) {
						oViewModel.setProperty("/S03", oData);
					},
					filters: this._mFilters.S03
				});
		...
		_showObject: function(oItem) {
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("IssueId")
			});
		},
